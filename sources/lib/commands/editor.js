...
export default {
	...
	i: withoutGoto((editor, { instance }) => {
		editor
			.getSelections()
			.forEach(selection =>
				selection.cursor.setBufferPosition([
					selection.getBufferRange().start.row,
					selection.getBufferRange().start.column,
				]),
			);
	 ...
	 */
	a: withGoto(
		(editor, { dispatch }) => dispatch("fuzzy-finder:toggle-buffer-finder"),
		{},
		(editor, { instance }) => {
			editor
				.getSelections()
				.forEach(selection =>
					selection.cursor.setBufferPosition([
						selection.getBufferRange().end.row,
						selection.getBufferRange().end.column,
					]),
				);
			instance.changeModeToInsert();
		},
