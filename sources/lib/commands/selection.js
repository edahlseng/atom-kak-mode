...
export default {
	 ...
	 * @selections -
	 * @shift insert at line beginning
	 */
	i: withGoto(
		(selection, { withShift }) => {
			withShift
	...
	backspace: (selection, { instance }) => {
		...
		if (count) {
			...
			const updatedCount = updatedCountString
				? parseInt(updatedCountString)
				: 0;
	...
	";": selection => {
		...
		const cursorAtBeginningOfSelection = isCursorAtBeginningOfSelection(
			selection.cursor,
		);
		selection.clear();
