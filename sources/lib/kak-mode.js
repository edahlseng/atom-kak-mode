...
import {
	isCursorAtBeginningOfSelection,
	updateString,
} from "./commands/utils/index";
...
export default {
	...
	onMouseDownOnTextEditor(e) {
		...
		const screenPosition = e.currentTarget.component.screenPositionForMouseEvent(
			e,
		);
		editor.setSelectedScreenRange(
			new Range(
				screenPosition,
				getScreenPositionToRight(editor, screenPosition),
			),
		);
	...
	addMouseEvents(editor) {
		if (editor) {
			editor.editorElement.addEventListener(
				"mousedown",
				this.onMouseDownOnTextEditor.bind(this),
			);
		}
	},
	...
	updateCursorStyles(editor) {
		...
		editor.getSelections().forEach(selection => {
			...
			if (oldDecorations[selection.cursor.marker.id]) {
				this.decorations[selection.cursor.marker.id] =
					oldDecorations[selection.cursor.marker.id];
			} else {
				this.decorations[selection.cursor.marker.id] = editor.decorateMarker(
			...
			if (isCursorAtBeginningOfSelection(selection.cursor)) {
				this.decorations[selection.cursor.marker.id].setProperties({
					type: "cursor",
					class: "at-beginning-of-selection",
				});
			} else {
				this.decorations[selection.cursor.marker.id].setProperties({
					type: "cursor",
					class: "at-end-of-selection",
				});
			}
		});
	},
	...
	activate() {
		...
		const getKeyHandler = (key, modifier) => ({
			...
			didDispatch: e => {
				...
				} else if (this.state.replacing) {
					editor.getSelections().forEach(selection => {
						const startingText = selection.getText();
						const newText = key.repeat(startingText.length);
		...
		this.subscriptions.add(
			atom.workspace.observeTextEditors(this.addMouseEvents.bind(this)),
		);
